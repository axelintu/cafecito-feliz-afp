...
- Dado que hay un carrito con tres productos: `2 x Café Americano ($40)`, `1 x Café Latte ($45)`, `1 x Té Verde ($35)` y los botones para aumentar cantidad `+` están activos para los tres productos, y los botones para disminuir cantidad `-` están desactivados para Café Latte, y para Té Verde, cuando el vendedor escribe la cantidad de Café Americano como `1` y sale del campo, entonces el botón `-` de Café Americano se deshabilita y quedan deshabilitados los tres botones de `-`.
- Dado que hay un carrito con tres productos: `2 x Café Americano ($40)`, `1 x Café Latte ($45)`, `1 x Té Verde ($35)` y los botones para aumentar cantidad `+` están activos para los tres productos, y los botones para disminuir cantidad `-` están desactivados para Café Latte, y para Té Verde, cuando el vendedor da clic en el botón de `-` del Café Americano y baja a `1`, entonces el botón `-` de Café Americano se deshabilita y quedan deshabilitados los tres botones de `-`.
- Dado que el stock en el catálogo para Café Americano es `6`, y hay un carrito con un producto: `5 x Café Americano ($40)`, y su botón para aumentar cantidad `+` está activo, cuando el vendedor da clic en el botón de `+` del Café Americano y sube a `6`, entonces el botón `+` de Café Americano se deshabilita.
- Dado que el stock en el catálogo para Café Americano es `6`, y hay un carrito con un producto: `5 x Café Americano ($40)`, y su botón para aumentar cantidad `+` está activo, cuando el vendedor escribe en el campo de cantidad del Café Americano el número `6` y sale del campo, entonces el botón `+` de Café Americano se deshabilita.
- Dado que el stock en el catálogo para Café Americano es `6`, y hay un carrito con un producto: `5 x Café Americano ($40)`, y su botón para aumentar cantidad `+` está activo, cuando el vendedor escribe en el campo de cantidad del Café Americano el número `7` y sale del campo, entonces el frontend borra el `7` y vuelve al `5` y muestra el mensaje "No puedes vender una cantidad mayor al stock".
...
Historia 5 (backlog: #7)

Como vendedor quiero ver el subtotal, el descuento aplicado y el total de la compra para cobrarle al cliente el monto correcto y poder explicarle su descuento.

Criterios:

- Dado que el POS tiene el registro del cliente `Rocío Miranda` con `3 compras` y el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor enlaza el carrito con `Rocío Miranda`, entonces el frontend muestra `Subtotal: $125.00`, `descuento: $6.25 - (5%, 3 compras acumuladas de cliente frecuente)` y el `Total: $118.75`.
- Dado que el POS tiene el registro del cliente `Rocío Miranda` con `4 compras` y el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor enlaza el carrito con `Rocío Miranda`, entonces el frontend muestra `Subtotal: $125.00`, `descuento: $12.50 - (10%, 4 compras acumuladas de cliente frecuente)` y el `Total: $112.50`.
- Dado que el POS tiene el registro del cliente `Rocío Miranda` con `7 compras` y el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor enlaza el carrito con `Rocío Miranda`, entonces el frontend muestra `Subtotal: $125.00`, `descuento: $12.50 - (10%, 7 compras acumuladas de cliente frecuente)` y el `Total: $112.50`.
- Dado que el POS tiene el registro del cliente `Rocío Miranda` con `8 compras` y el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor enlaza el carrito con `Rocío Miranda`, entonces el frontend muestra `Subtotal: $125.00`, `descuento: $18.75 - (15%, 8 compras acumuladas de cliente frecuente)` y el `Total: $106.25`.
- Dado que el POS tiene el registro del cliente `Rocío Miranda` con `0 compras` y el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor enlaza el carrito con `Rocío Miranda`, entonces el frontend muestra `Subtotal: $125.00`, `descuento: $0.00 - (0%, 0 compras acumuladas. Aún no hay descuento de cliente frecuente)` y el `Total: $125.00`.
- Dado que el vendedor tiene en carrito de venta `2 x Café Americano ($40)` y `1 x Café Latte ($45)`, cuando el vendedor no enlaza el carrito con ningún cliente, entonces el frontend muestra el mensaje: `Para obtener descuento de cliente frecuente agrega al cliente`, un botón de `Agregar cliente`, el `Subtotal: $125.00`, y el `Total: $125.00`.
